...
export const TableBase: React.FC<TableProps> = ({
  ...
  <div className={`w-full overflow-x-auto rounded-xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xs custom-scrollbar ${stickyHeader ? 'max-h-[600px] overflow-y-auto' : ''}`}>
  ...
  <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mb-4">
    ...
    {onSearchChange && (
      <div className="relative w-full sm:w-72">
        ...
        <input
          ...
          className="w-full pl-9 pr-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 shadow-2xs"
        />
      </div>
    )}
    ...
    <div className="flex items-center gap-2 w-full sm:w-auto justify-end flex-wrap">
      ...
      {onExport && (
        <button
          onClick={onExport}
          className="px-3 py-2 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700/80 transition-all flex items-center gap-1.5 shadow-2xs"
        >
      ...
      {onRefresh && (
        <button
          ...
          className="p-2 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/80 transition-all shadow-2xs"
          title="Refresh Data"
        >
    ...
    <div className="relative inline-block text-left">
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 mt-1 w-44 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl py-1 z-50 animate-in fade-in zoom-in-95 duration-150">
            {onView && (
              <button
                onClick={(e) => {
                  e.stopPropagation();
